# -*- coding: utf-8 -*-
"""ANN_A-BCD_opt2.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1Iu9K5CCiP9SLUoLVkYEf4s-EB2VXL7q3

This is the msot updated version of the code. It filters data for both purity and recovery simultaneously. It also calculates HK in top and LK in bottom.
"""
...
plt.ylabel(y)
plt.legend()
plt.xticks(range(int(min(dt_short.loc[:,'Sample'])), int(max(dt_short.loc[:,'Sample'])), 2))
...
dt = dt.dropna()
dt = dt.drop(labels='class',axis=1)
...
dt['HK_top'] = dt['HK_top'] * 28.05 / 42.08         # kg B/ kg TOP -> kmol B/ kmol TOP (el shortcut de Aspen HYSYS llegeix en kmol)
dt['LK_bottom'] = dt['LK_bottom'] * 57.15 / 28.05   # kg A/ kg BOTTOM -> kmol A/ kmol BOTTOM (el shortcut de Aspen HYSYS llegeix en kmol)
...
dt_useless = dt
dt = dt.loc[:,['NT','RR','Distillate_Rate','HK_top', 'LK_bottom', 'TOP_Fraction_A','Recovery','Cost/kgTOP']]

dt['Distillate_Rate'] = dt['Distillate_Rate']*3600   # kg/s -> kg/h
...
Costs['%Cond']   = C_Cond*amort/ (dt['Distillate_Rate']* Sec_Year/3600)
Costs['%Reb' ]   = C_Reb*amort/ (dt['Distillate_Rate']* Sec_Year/3600)
Costs['%Trays']  = C_Trays*amort/ (dt['Distillate_Rate']* Sec_Year/3600)
Costs['%Column'] = C_Column*amort/ (dt['Distillate_Rate']* Sec_Year/3600)
Costs['%HU']     = C_HU/ (dt['Distillate_Rate']* Sec_Year/3600)
Costs['%CU']     = C_CU/ (dt['Distillate_Rate']* Sec_Year/3600)

Costs['%TOTAL']  = C_TOTAL/ (dt['Distillate_Rate']* Sec_Year/3600)
...
"""# Filter data (recovery and top fraction A)
...
dt = dt.sort_values(['TOP_Fraction_A','Recovery'],axis=0, ascending=False)
dt.head()

dt = dt.loc[dt.groupby(['TOP_Fraction_A','Recovery'])['Cost/kgTOP'].idxmin()]
dt = dt.sort_values(['TOP_Fraction_A','Recovery'],axis=0, ascending=False)
...
""" Cheapest option WITHOUT constraints"""
Cost_min_filt = pd.DataFrame(dt.iloc[getIndexes(dt,dt.iloc[:,-1].min()), :])
Cost_min_filt

""" Cheapest option WITH constraints"""
Rec = dt[dt['Recovery']>= 0.99]
Rec_Pur = Rec[Rec['TOP_Fraction_A']>= 0.99]
Rec_Pur.index = range(len(Rec_Pur))

Cost_min_filt = pd.DataFrame(Rec_Pur.iloc[getIndexes(Rec_Pur,Rec_Pur.iloc[:,-1].min()), :])
Cost_min_filt

"""## Heat Maps"""

dt.to_csv('/content/drive/MyDrive/Quatrimestre 8/Dades i codi/Data/Filter')

HeatMap3D(dt.loc[:,['NT','RR','Distillate_Rate','TOP_Fraction_A']])

HeatMap3D(dt.loc[:,['NT','RR','Distillate_Rate','Recovery']])

HeatMap3D(dt.loc[:,['NT','RR','Distillate_Rate','Cost/kgTOP']])

"""# Divide into a training and testing set. Normalize"""

HK_top = dt.loc[:,'HK_top']
LK_bottom = dt.loc[:,'LK_bottom']
...
model = Sequential([
      Dense(3, input_shape= (input_variables,), activation= 'relu'),    #Adding first layer (number of nodes, inputs(number of variables, activation function))
      Dense(2  , activation= "relu"),                                   #Adding hidden layer 
...
print("Computational time: ",round(t.elapsed,2)," sec")

model.save('/content/drive/MyDrive/Quatrimestre 8/Dades i codi/Data/ANN_model_opt.keras')
...
plt.scatter(x_testR.iloc[:,X],  residuals_testR.iloc[:,-1],  s=100,marker='.', c='g', label='Validation set')
...
plt.plot([y_testR.min(),y_testR.max()],[y_testR.min(),y_testR.max()], color='red', label='Ideal Value')
...
fig = plt.figure(figsize=(10,10))
ax = fig.add_subplot(111,projection='3d')
ax.scatter3D(Data_2D.iloc[:,1], Data_2D.iloc[:,2], Data_2D.iloc[:,-1], alpha=1)
#ax.scatter(x_trainR.iloc[:,1], x_trainR.iloc[:,2], y_trainR.iloc[:,-1], alpha=0.1, s=30, c='m', label='Training set')
ax.scatter(x_testR.iloc[:,1],  x_testR.iloc[:,2],  y_testR.iloc[:,-1],  alpha=0.5, s=30, c='g', label='Validation set')
...
model.save('/content/drive/MyDrive/Quatrimestre 8/Dades i codi/A-BCD_ANN_model.keras')
"""
...
from tensorflow import keras
model_sim = keras.models.load_model('/content/drive/MyDrive/Quatrimestre 8/Dades i codi/Data/ANN_model_sim.keras')

model_sim.summary()

LOSS_sim= round(model_sim.evaluate(x_test, y_test, verbose=0)[0],4)
LOSS_sim

y_model_sim = pd.DataFrame(model_sim.predict(x_model))

grip_model['Simulation']= round(pd.DataFrame((y_model_sim) * nrm_std[-1] + nrm_mean[-1]),4)
grip_model = grip_model.rename(columns={'Cost/kgTOP':'Optimization'})               # Change the name of the column

grip_model.head()

fig = plt.figure(figsize=[6,6])
plt.scatter(y_model*nrm_std[-1]+nrm_mean[-1], y_model_sim*nrm_std[-1]+nrm_mean[-1],  s=5, alpha=0.8, marker='.', c='blue', label='Real values')
#plt.plot(y_model, regression, color='red', label='Regression')
plt.plot([y_model.min()*nrm_std[-1]+nrm_mean[-1],y_model.max()*nrm_std[-1]+nrm_mean[-1]],[y_model.min()*nrm_std[-1]+nrm_mean[-1],y_model.max()*nrm_std[-1]+nrm_mean[-1]] , linewidth=3, color='red', label='Ideal values')
plt.xlabel('Optimization Model', size=12)
plt.ylabel('Simulation Model', size=12)
plt.title('Comparsion of both models', size=16)
plt.legend(fontsize=12)
plt.grid()
plt.show()

R2_res = round(r2_score(grip_model.iloc[:,-2],grip_model.iloc[:,-1]),4)
R2_res